...
if( $task eq 'read' ){
	...
	foreach my $dir ( @dirs ){
		...
		my $output = "${whereami}/templates/cospend.pot";
		...
		foreach my $file ( @totranslate ){
            ...
            #print "    Reading $file\n";
			`xgettext --output="$output" $joinexisting $keywords --language=$language "$file" --add-comments=TRANSLATORS --from-code=UTF-8 --package-version="8.0.0" --package-name="Cospend app" --msgid-bugs-address="ju-translations\@cassio.pe"`;
		}
	...
	foreach my $dir ( @dirs ){
		...
		foreach my $language ( @languages ){
			...
			my $input = "${whereami}/$language/cospend.po";
			next unless -e $input;
